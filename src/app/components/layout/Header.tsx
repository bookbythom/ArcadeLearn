  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[60] backdrop-blur-lg bg-[rgba(55,55,55,0.2)] border-b border-[rgba(255,255,255,0.1)]">
      <div className="w-full px-3 max-[380px]:px-2 sm:px-6 lg:px-8">
        <div className="flex flex-wrap sm:flex-nowrap items-center justify-between min-h-16 max-[380px]:min-h-14 py-2 max-[380px]:py-1.5 sm:py-0 gap-2 sm:gap-4">
          {/* Lava strana: Level a Streak */}
          <div className="flex items-center gap-4 max-[380px]:gap-3 sm:gap-8 lg:gap-12 order-1">
            <div className="flex items-center gap-2 max-[380px]:w-[108px] w-[124px] sm:w-[140px]">
              <div className="relative flex items-center gap-2 flex-1">
                <div className="absolute right-2 text-[#7f7f7f] text-sm max-[380px]:text-xs sm:text-base lg:text-lg font-medium z-10 pointer-events-none">
                  {nextLevel}
                </div>
                <div className="flex-1 h-6 max-[380px]:h-5 sm:h-7 border-2 border-[#7f7f7f] rounded-full relative overflow-hidden">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center text-white text-xs max-[380px]:text-[10px] font-medium z-10 pl-2">
                    LVL.{currentLevel}
                  </div>
                </div>
              </div>
            ...
            <button 
              className="flex items-center gap-1.5 max-[380px]:gap-1 sm:gap-2 hover:scale-110 transition-transform"
              onClick={handleStreakClick}
            >
              <div className={streakIconClass}>
                <StreakIcon />
              </div>
              <span className={streakTextColor + ' text-base max-[380px]:text-sm sm:text-lg lg:text-xl font-medium'}>
                {props.streakCount}
              </span>
            </button>
          </div>
          ...
          {/* Stred: Navigacia */}
          <nav className="flex items-center justify-center gap-6 max-[380px]:gap-4 sm:gap-10 lg:gap-12 w-full sm:w-auto order-3 sm:order-2 sm:absolute sm:left-1/2 sm:-translate-x-1/2 pb-1 sm:pb-0">
            <button 
              onClick={() => {
                props.onTabChange("home");
              }}
              onMouseEnter={() => props.onTabHover?.("home")}
              className={'text-base max-[380px]:text-sm sm:text-lg lg:text-xl transition-colors ' + homeTabColor}
            >
              Home
            </button>
            <button 
              ...
              className={'text-base max-[380px]:text-sm sm:text-lg lg:text-xl transition-colors ' + mistakesTabColor}
            >
              Mistakes
            </button>
            {props.isAdmin ? (
              <button 
                ...
                className={'text-base max-[380px]:text-sm sm:text-lg lg:text-xl transition-colors ' + adminTabColor}
              >
                Admin
              </button>
            ) : null}
          ...
          >
            ...
            <span className="text-[#b6b6b6] text-sm max-[380px]:text-xs sm:text-base max-w-[90px] max-[380px]:max-w-[72px] sm:max-w-none truncate">{props.userProfile.name}</span>
            {props.userProfile.profilePicture ? (
              <img 
                src={props.userProfile.profilePicture} 
                alt={props.userProfile.name} 
                className="w-8 h-8 max-[380px]:w-7 max-[380px]:h-7 sm:w-9 sm:h-9 rounded-full object-cover flex-shrink-0" 
              />
            ) : (
              <div className="w-8 h-8 max-[380px]:w-7 max-[380px]:h-7 sm:w-9 sm:h-9 bg-[#c4c4c4] rounded-full flex-shrink-0" />
            )}
          </button>
        </div>
      </div>
