...
export default function XPProgressIndicator({ userProgress, currentLevel, isVisible, islandProgress }: XPProgressIndicatorProps) {
  ...
  if (sectionLocked) {
    ...
    return (
      <div 
        className="fixed left-2 right-2 sm:left-4 sm:right-auto lg:left-8 z-50 backdrop-blur-lg bg-[rgba(28,28,30,0.95)] border border-[rgba(255,255,255,0.2)] rounded-lg shadow-2xl p-3 max-[430px]:p-2 max-[390px]:p-1.5 sm:p-4 lg:p-5 w-auto sm:w-[calc(100vw-1.5rem)] max-w-[340px] max-[430px]:max-w-[270px] transition-opacity duration-300"
        style={{
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="text-xl max-[430px]:text-base sm:text-2xl font-bold text-gray-300">
            {levelName} Locked
          </div>
        </div>

        {/* Sprava */}
        <div className="text-sm max-[430px]:text-[11px] text-gray-300">
  ...
  return (
    ...
    >
      {/* Zobrazenie XP */}
      <div className="mb-3 max-[430px]:mb-2">
        <div className="flex items-baseline justify-between mb-3 max-[430px]:mb-2">
          <span className="text-3xl max-[430px]:text-xl sm:text-4xl lg:text-5xl font-bold text-white transition-all duration-300">
            {xpInCurrentLevel}
          </span>
          <span className="text-sm max-[430px]:text-[11px] sm:text-base lg:text-lg text-gray-400">
            / {FINAL_TEST_XP} XP
          </span>
        </div>
      ...
      {/* Stavova sprava */}
      {finalTestUnlocked ? (
        <div className="flex items-center gap-2 max-[430px]:gap-1 text-green-400 text-sm max-[430px]:text-[11px] transition-all duration-300">
          <svg className="w-4 h-4 max-[430px]:w-3.5 max-[430px]:h-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
          </svg>
          <span className="font-medium">Final Test Unlocked!</span>
        </div>
      ) : (
        <div className="text-sm max-[430px]:text-[11px] text-gray-300 transition-all duration-300">
